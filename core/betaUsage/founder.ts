...
export function founderUsageResult(
  ...
  const users = (data ?? []).map((row): FounderBetaUsageRow => {
    ...
    activatedAt,
    activationStatus: activatedAt ? "ACTIVATED" : "REGISTERED_ONLY",
    lifecycleStatus: classifyLifecycle(activatedAt ? 1 : 0, row.last_active_at, now),
  ...
  const activationMinutes = users.flatMap((user) => user.activatedAt
    ? [(Date.parse(user.activatedAt) - Date.parse(user.registeredAt)) / 60_000] : [])
    .filter((value) => Number.isFinite(value) && value >= 0).sort((a, b) => a - b);
  const middle = Math.floor(activationMinutes.length / 2);
  const medianMinutesToActivation = activationMinutes.length === 0 ? null
    : activationMinutes.length % 2 === 1 ? activationMinutes[middle]
      : (activationMinutes[middle - 1] + activationMinutes[middle]) / 2;
